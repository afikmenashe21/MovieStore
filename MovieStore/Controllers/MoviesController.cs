...
namespace MovieStore.Controllers
{
    public class MoviesController : Controller
        ...
        public async Task<IActionResult> Search ( string name )
            ...
            await combinedSearch(null, null, 0, 160, 0, 7);
            await CreateMovie(movie, resultTitle);//Creates reviews either
            //Display list of movies with simillar names
        ...
        private async Task combinedSearch(DateTime? fromDate = null, DateTime? toDate = null, int fromDuration = 0, int toDuration = 999, int fromRating = 0, int toRating = 10)
        {
            movies.Clear();
            if (fromDate == null)
            ...
            foreach (Movie movie in _context.Movie)
            {
                if (movie.Duration >= fromDuration && movie.Duration <= toDuration
                    && movie.ReleaseDate >= fromDate && movie.ReleaseDate <= toDate
                    && movie.AverageRating >= fromRating && movie.AverageRating <= toRating)
                {
                    movies.Add(movie);
                }
            }
